...
#ifndef COMMON_AGENTS_HOLONOMICAGENT_HPP_
#define COMMON_AGENTS_HOLONOMICAGENT_HPP_

#include <planner_interfaces/Agent.hpp>
#include <planner_interfaces/FclSupport.hpp>

#include "HolonomicState.hpp"
#include "HolonomicSystemInput.hpp"

namespace common_agents {

class HolonomicAgent : public planning2d::Agent {

 public:
  typedef HolonomicState State;
  typedef HolonomicSystemInput SystemInput;
  typedef HolonomicStateStamped StateStamped;
  typedef HolonomicSystemInputStamped SystemInputStamped;

  PLANNING_2D_POINTER_TYPEDEFS(HolonomicAgent);

 public:
  HolonomicAgent();
  HolonomicAgent(bool isControllable,
                 const planning2d::Id id,
                 const HolonomicStateStamped& stateStamped,
                 planning2d::CollisionGeometryConstPtr geometry);
  virtual ~HolonomicAgent();
   ...
   */
  void applyInput(const planning2d::SystemInput& input, planning2d::Duration dt) override;

  //! Returns const reference to the current stamped state of the agent (stamp + pose + state variables)
  const HolonomicStateStamped& stateStamped() const override { return _stateStamped; }

  //! Returns mutable reference to the stamped state of the agent (stamp + pose + state variables)
  HolonomicStateStamped& stateStamped() override { return _stateStamped; }
  ...
  //! deep copy
  planning2d::Agent::Ptr clone() const override { return planning2d::Agent::Ptr(new HolonomicAgent(*this)); }
 ...
 private:
  HolonomicStateStamped _stateStamped;
